...
impl Cfg1 {
    ...
    #[doc = "PDM Data selection. This bit controls the data source for I2S transmit, and cannot be set in Rx mode. This bit only has an effect if the device the Flexcomm resides in includes a D-Mic subsystem. For the LPC5411x, this bit applies only to Flexcomm 7."]
    #[inline(always)]
    pub const fn set_pdmdata(&mut self, val: super::vals::Pdmdata) {
...
impl defmt::Format for Cfg1 {
    fn format(&self, f: defmt::Formatter) {
        defmt::write!(
            f,
            "Cfg1 {{ mainenable: {=bool:?}, datapause: {:?}, paircount: {:?}, mstslvcfg: {:?}, mode: {:?}, rightlow: {:?}, leftjust: {:?}, onechannel: {:?}, pdmdata: {:?}, sck_pol: {:?}, ws_pol: {=bool:?}, datalen: {=u8:?} }}",
            self.mainenable(),
            self.datapause(),
            self.paircount(),
            self.mstslvcfg(),
...
#[cfg(feature = "defmt")]
impl defmt::Format for Pcfg1 {
    fn format(&self, f: defmt::Formatter) {
        defmt::write!(
            f,
            "Pcfg1 {{ pairenable: {=bool:?}, onechannel: {=bool:?} }}",
            self.pairenable(),
            self.onechannel()
        )
    }
...
#[cfg(feature = "defmt")]
impl defmt::Format for Pcfg2 {
    fn format(&self, f: defmt::Formatter) {
        defmt::write!(f, "Pcfg2 {{ position: {=u16:?} }}", self.position())
    }
}
...
impl Pstat {
    ...
    #[doc = "Left/Right indication."]
    #[inline(always)]
    pub const fn set_lr(&mut self, val: bool) {
        self.0 = (self.0 & !(0x01 << 2usize)) | (((val as u32) & 0x01) << 2usize);
    }
...
impl defmt::Format for Pstat {
    fn format(&self, f: defmt::Formatter) {
        defmt::write!(
            f,
            "Pstat {{ busy: {=bool:?}, slvfrmerr: {=bool:?}, lr: {=bool:?}, datapaused: {=bool:?} }}",
            self.busy(),
            self.slvfrmerr(),
            self.lr(),
            self.datapaused()
